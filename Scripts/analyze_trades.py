...
def analyze_combinations(csv_file, investment_per_trade=200000, risk_free_rate=0.05):
    ...
    # Group by RSI & SL%
    for (rsi, sl), group in df.groupby(["rsi", "sl_pct"]):
        group = group.copy()
...
def analyze_group(group, investment_per_trade=200000, risk_free_rate=0.06):
    group = group.copy()
    group["holding_days"] = (group["exit_date"] - group["entry_date"]).dt.days + 1
    group["return"] = group["pnl"] / investment_per_trade
...
def analyze_all_trades(folder_path):
    ...
    for file in os.listdir(folder_path):
        if file.endswith("trades.csv"):
            ...
            if symbol == 'NIFTY50':
                continue
            df = pd.read_csv(os.path.join(folder_path, file), parse_dates=["entry_date", "exit_date"])
            print(symbol)
            #Group RSI x SL%
            for (rsi, sl), group in df.groupby(["rsi", "sl_pct"]):
    ...
    summary_df = summary_df.sort_values(by="cagr", ascending=False).reset_index(drop=True)
    summary_df.to_csv("/Users/umanggohil/Documents/BGYRT/dev-work/General/scripts/trades_analysis/nifty500_single_summary.csv", index=False)
    return summary_df

summary_df = analyze_all_trades("/Users/umanggohil/Documents/BGYRT/dev-work/General/onservations/symbol_trades_single")
